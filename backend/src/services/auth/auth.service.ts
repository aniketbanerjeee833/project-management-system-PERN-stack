...
export async function registerService(name: string, email: string, password: string) {
  const existing = await findUserByEmail(email);
  if (existing) throw new AppError("Email is already registered.", 409); // 409 Conflict

  const password_hash = await bcrypt.hash(password, SALT_ROUNDS);
  const user  = await insertUser(name, email, password_hash);
  const token = signToken(user.id);

  return { token, user: safeUser(user), needsWorkspace: true };
}

// ── login ─────────────────────────────────────────────────────────────────────
export async function loginService(email: string, password: string) {
  const user = await findUserByEmail(email);
  if (!user) throw new AppError("Invalid email or password.", 401); // 401 Unauthorized
  ...
  return {
    user: safeUser(user),
    workspace: me ? { id: me.workspace_id, name: me.workspace_name, slug: me.workspace_slug } : null,
    role: me?.role ?? null,           // ← this is what the frontend needs
    needsWorkspace: !me,              // true if admin hasn't created workspace yet,
    token
  };
...
// ── create workspace ──────────────────────────────────────────────────────────
export async function createWorkspaceService(name: string, slug: string, owner_id: number) {
  const taken = await slugExists(slug);
  if (taken) throw new AppError("This workspace URL is already taken.", 409);
...
export async function getMeService(user_id: number) {
  const me = await findMeByUserId(user_id);
  if (!me) return { needsWorkspace: true };

  return {
    needsWorkspace: false,
    user:      { id: me.user_id, name: me.name, email: me.email, avatar_url: me.avatar_url },
    workspace: { id: me.workspace_id, name: me.workspace_name, slug: me.workspace_slug },
    role:      me.role,
  };
}