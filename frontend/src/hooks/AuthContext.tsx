...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchMe = useCallback(async (): Promise<void> => {
    try {
      ...
      if (me.needsWorkspace) {
        // user exists in DB but has no workspace yet (fresh admin)
        setIsAuthenticated(true);
        setNeedsWorkspace(true);
        setIsLoading(false);
        return;
      ...
      // ── 401 = no session / not logged in yet — this is NORMAL on page load ──
      // Do NOT throw here. Just mark as unauthenticated and move on.
      const status = (err as { response?: { status?: number } })?.response?.status;

      if (status === 401) {
        // expected — user hasn't logged in
        setIsAuthenticated(false);
        setUser(null);
        setWorkspace(null);
        setRole(null);
      } else {
        // unexpected server error — still don't crash, just log it
        console.error("Unexpected error fetching /auth/me:", err);
        setIsAuthenticated(false);
      }
    } finally {
  ...
  // ── setAuth — called after login / invite accept ───────────────────────────
  ...
  // ── logout — useCallback fixes the infinite loop ───────────────────────────
